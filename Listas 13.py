...
def x_antes(words):
    ...
    for word in words:
        if word[0] == 'x':
            ...
            outras_letras.append(word)
    return sorted(x) + sorted(outras_letras) # sorted faz ordenação das palavras
...
def sort_last(tuples):
  return sorted(tuples, key = last) # ordena as tuplas de acordo com ultimo elemento da tupla
