...
def busca(frase, palavra):
    ...
    for i in range(len(frase) - len(palavra) + 1):
        # por meio do range, percorre todos os índices onde a palavra cabe; 
        # len(frase) - len(palavra) calcula o último índice válido; 
        # +1 garante que a última posição seja incluída.
        if frase[i:i + len(palavra)] == palavra:
            # pega uma fatia da frase do tamanho da palavra e compara com ela.
            contador += 1
    return contador                           
