...
def cripto(frase):
    palavras = frase.split()  # SPLIT separa palavras
    frase_final = ""
    for palavra in palavras:
        palavra_modificada = sorted(set(palavra))  # SET retira letras repetidas e SORTED ordena letras por ordem alfabética
        frase_final += "".join(palavra_modificada) + " " # JOIN junta letras de uma palavra
    return frase_final.strip()  # STRIP remove espaço no final
...
def derivada(coef):
    resultado = []
    for grau, c in enumerate(coef):
        if grau != 0:
            resultado.append(c * grau)
    return resultado
...
def soma(n1, n2):
    resultado = []
    vai_um = 0
    for a, b in zip(n1, n2):  # percorre simultaneamente os dígitos das duas listas
        total = a + b + vai_um  # soma dígitos atuais mais vai_um (5 + 9 + 0 = 14)
        resultado.append(total % 10)  # adiciona resto da divisão no resultado (14 -> 4)
        vai_um = total // 10  # adiciona parte inteira da divisão (vai_um) na proxima soma (11 -> 1)
    if vai_um > 0:
        resultado.append(vai_um)  # adiciona vai_um final no resultado, se sobrar
    return resultado
# Saída esperada: ([5, 2, 3, 4], [9, 8, 7, 8], [4, 1, 1, 3, 1]) --> 4325 + 8789 = 13114
...
def main():
    ...
    test(
        cripto("Batatinha quando nasce esparrama pelo chão"),
